export const MOVE_DIRECTION_DID_CHANGE = 'moveDirectionDidChange';

export const PLAYER_SHIP_ID_DID_CHANGE = 'playerShipIdDidChange';

export const POWERUPS_DID_CHANGE = 'powerupsDidChange';

export const PROJECTILES_DID_CHANGE = 'projectilesDidChange';

export const SHIPS_DID_CHANGE = 'shipsDidChange';

export const TIME_DID_CHANGE = 'timeDidChange';
